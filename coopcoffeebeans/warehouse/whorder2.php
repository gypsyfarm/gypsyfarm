...
<?PHP
 echo "<form name='frmMain' action=whorder2.php?order_id=".$order_id." method=post>";
?>
...
<font size=3><a href="whorder1.php">Process more Orders</a></font><br>
<font size=3><a href="../logout.php">Log out</a></font><br>
</td> 
 
 <td colspan=2 align=center>
 <h2>
Cooperative Coffee Warehouse Processing Page
...
# connect to database
 ...
 if ($subbtn == "Shipped") {
   mysql_select_db($tbl_order_header);
    ...
    echo "<font color=red><b> Order has been shipped by $valid_user </b></font> ";
  ...
  $message = "Warehouse ".$valid_user." has shipped order ".$order_nbr." for ".$shipaddress1;

  mail( "cgeeck@dupuystorage.com,alorion@affiliated.ca,pnewberry@hfhi.org,abby@coopcoffees.com", "Testing New Order Email",
    $message, "From: $email" );
...
# this sections gets all information for the order being displayed on the invoice
  ...
  $query = 'select oh.header_id, oh.STATUS, oh.order_nbr, li.item_id, li.lot_id, ci2.lot_ship,oh.customer_key,'.
           ...
           'ci.item_description, ci2.mark, ci2.warehouse, ci2.warehouse_code, oh.order_date, oh.ship_date, '.
...
<tr><td>
<INPUT name=subbtn TYPE="SUBMIT" value="Shipped">
</td>
<td>
&nbsp;
</td>
...
<br>
 Please call (229)924-2468 if there is any questions or problems concerning
...
<?
  echo $row['order_nbr'];    
  echo '<input type=hidden name=order_nbr value="'.$row['order_nbr'].'">';   
  echo '<input type=hidden name=shipaddress1 value="'.$row['ShipAddress1'].'">';  
  echo '<input type=hidden name=warehouse value="'.$row['warehouse'].'">';
?>
<br>
</td>
<tr>
...
<?
  ...
  $fbo_city = $row['fbo_city'];
  $truck = $row['truck'];
  $ftr_charges = $row['frt_charges'];
...
# end for loop.
  ...
  echo '<tr>';
  echo '<td colspan=3>';
  echo '<b>FBO City:&nbsp;&nbsp;&nbsp;</b><br>'.$fbo_city.'&nbsp;<br>';
  echo '</td>';
  echo '<td colspan=3>';
