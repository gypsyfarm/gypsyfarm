...
echo '</td><td align=center>';
echo '<h3>Contracted and Landed Report </h3>';
...
/*
                     ...
                     SELECT id.item_code,   ci.STATUS, Month(ci.ship_date),  sum(ci.quantity) as initial_quantity,  
          sum(ci.transfer_in) as transfer_in , sum(ci.transfer_out) as transfer_out    
          FROM  item_description id,  coop_item ci
          WHERE id.item_code = ci.item_code
           and ci.quantity > 0
           AND ci.ship_date Between '2008-01-01' and '2008-12-31'
          group by id.item_code,  ci.STATUS, Month(ci.ship_date)
              order by id.item_code, ci.STATUS, Month(ci.ship_date) 
              
              
                        
           SELECT id.item_code,   ci.item_description, sum(ci.quantity) as initial_quantity,  
          sum(ci.transfer_in) as transfer_in , sum(ci.transfer_out) as transfer_out    
          FROM  item_description id,  coop_item ci
          WHERE id.item_code = ci.item_code
           and ci.quantity > 0
           AND ci.ship_date Between '2008-01-01' and '2008-12-31'
          group by id.item_code,  ci.item_description 
...
echo '<th align=center><font size=2 color=blue>YTD Sales</font></th>';
