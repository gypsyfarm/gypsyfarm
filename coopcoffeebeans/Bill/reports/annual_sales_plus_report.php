...
// check session variable
  ...
  header("Location: http://www.coopcoffeesbeans.com/badlogin.php");
    ...
    echo'<BODY text=#000000 vLink=#800080 aLink=#ff0000 link=#0000ff bgColor=#ffffff>';
      ...
      echo '<form method=POST action=annual_sales_plus_report.php>';
...
echo '</td><td align=center>';
echo '<h3>Annual Monthly Sales Report </h3>';
echo "($company)";
...
//**************************Assign ranges to date for searching*********************
   ...
   $year_minus1 =  $year_range - 1;
   $year_minus2 =  $year_range - 2;
   $year_minus3 =  $year_range - 3;
 
     $total_total_minus1 =  0; 
  	 $total_total_minus2 =  0; 
  	 $total_total_minus3 =  0; 
   
    $from_date_minus1 = $year_minus1.'-01-01';
    $to_date_minus1 = $year_minus1.'-12-31';
    
    $from_date_minus2 = $year_minus2.'-01-01';
    $to_date_minus2 = $year_minus2.'-12-31';
    
    $from_date_minus3 = $year_minus3.'-01-01';
    $to_date_minus3 = $year_minus3.'-12-31';


/*

SELECT oi.item_code, Month( oh.order_date ) AS order_month, YEAR( oh.order_date ) , sum( li.quantity ) AS quantity
...
BETWEEN '2005-01-01'
AND '2008-12-31'
AND oh.customer_key =5
AND oh.STATUS <> 'I'
GROUP BY oi.item_code, YEAR( oh.order_date ) , Month( oh.order_date )
ORDER BY oi.item_code, Month( oh.order_date ) , YEAR( oh.order_date ) DESC 
*/

$query = "SELECT oi.item_code, Month(oh.order_date) as order_month, YEAR( oh.order_date ) as order_year , sum(li.quantity) as quantity
              FROM $tbl_order_header oh, $tbl_order_item oi, $tbl_coop_contact cc, $tbl_lot_item li
              WHERE oh.header_id = oi.header_key
              AND oh.customer_key = cc.contact_id
              and oi.item_id = li.item_id
              and oh.order_date Between '$from_date_minus3' 
              and '$to_date' 
...
echo "<th align=center><font size=2 color=blue>$year_minus3 </font></th>";
echo "<th align=center><font size=2 color=blue>$year_minus2 </font></th>";
echo "<th align=center><font size=2 color=blue>$year_minus1 </font></th>";
echo "<th align=center><font size=2 color=blue>$year_range YTD</font></th>";
...
$query_minus1 = "SELECT sum(li.quantity) as quantity
              FROM $tbl_order_header oh, $tbl_order_item oi, $tbl_coop_contact cc, $tbl_lot_item li
              WHERE oh.header_id = oi.header_key
              AND oh.customer_key = cc.contact_id
              and oi.item_id = li.item_id
              and oh.order_date Between '$from_date_minus1' 
              and '$to_date_minus1' 
              and oh.customer_key = $customer_key
              and oh.STATUS <> 'I' and oi.item_code = '$value'";
          
$query_minus2 = "SELECT sum(li.quantity) as quantity
              FROM $tbl_order_header oh, $tbl_order_item oi, $tbl_coop_contact cc, $tbl_lot_item li
              WHERE oh.header_id = oi.header_key
              AND oh.customer_key = cc.contact_id
              and oi.item_id = li.item_id
              and oh.order_date Between '$from_date_minus2' 
              and '$to_date_minus2' 
              and oh.customer_key = $customer_key
              and oh.STATUS <> 'I' and oi.item_code = '$value'";          
          
          
$query_minus3 = "SELECT sum(li.quantity) as quantity
              FROM $tbl_order_header oh, $tbl_order_item oi, $tbl_coop_contact cc, $tbl_lot_item li
              WHERE oh.header_id = oi.header_key
              AND oh.customer_key = cc.contact_id
              and oi.item_id = li.item_id
...
$result_minus1 = mysql_query($query_minus1, $db_conn);
$row_minus1 = mysql_fetch_array($result_minus1);
$amt_year_minus1 = $row_minus1["quantity"];

$result_minus2 = mysql_query($query_minus2, $db_conn);
$row_minus2 = mysql_fetch_array($result_minus2);
$amt_year_minus2 = $row_minus2["quantity"];

$result_minus3 = mysql_query($query_minus3, $db_conn);
$row_minus3 = mysql_fetch_array($result_minus3);
$amt_year_minus3 = $row_minus3["quantity"];
 
  if (!ISSET($amt_year_minus1)) {
  	$amt_year_minus1 = 0;
  } 
  
  if (!ISSET($amt_year_minus2)) {
  	$amt_year_minus2 = 0;
  } 
  
    if (!ISSET($amt_year_minus3)) {
  	$amt_year_minus3 = 0;
  } 
     
  	 $total_total_minus1 +=  $amt_year_minus1; 
  	 $total_total_minus2 +=  $amt_year_minus2; 
  	 $total_total_minus3 +=  $amt_year_minus3; 
      
            # now print the row !

      echo '<tr><td align=center>'.$value.'</td>';
      echo '<td align=right>'.$amt_year_minus3.'</td>';
      echo '<td align=right>'.$amt_year_minus2.'</td>';
      echo '<td align=right>'.$amt_year_minus1.'</td>';
...
#   echo " had sales of ".${$value}["sales"]."<br>";
 ...
 for ($i=0; $i <$num_results;  $i++)  {
   ...
   if (!in_array($row["item_code"],$item_code_array)) {
  	if ($heading == 0) {
          $heading = 1;
          echo '<tr bgcolor=palegreen><td align=left colspan=17><font color=blue>Commitments with No Sales:</font></td></tr>';  	
  	} 
...
*/ 
  ...
  echo '<td align=right><font color=blue>'.$total_total_minus3.'</font></td>';
  echo '<td align=right><font color=blue>'.$total_total_minus2.'</font></td>';
  echo '<td align=right><font color=blue>'.$total_total_minus1.'</font></td>';
