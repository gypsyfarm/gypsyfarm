...
function uploadcode(){ 
	...
	echo'<head>';
  	echo'<title>Contracted Green Report</title>';
	...
	#echo'<img SRC="../cclogo.jpeg" BORDER=0 height=122 width=133 align=LEFT>';
       ...
       echo '<form enctype="multipart/form-data" method=POST action="green_rpt_contracted.php"  method="post">';
...
echo 'View the <a href="green_rpt_future.php">Future Contracts </a> and the <a href="green_rpt.php">Green Report</a> for more green coffee deliveries info.';
echo '</td></tr></table><p>';
echo '<center>';
...
echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
echo '</span><span><font size=5 color=blue>Contract Green Report by Warehouse / Product Code </font></span><span"><br>';
echo '<font size=3 color=blue>Date Range: </font></span>'.$from_date.'<font size=3 color=blue> to </font>'.$to_date.'</center><br> ';
 
 if ($sort1 == 'del_dt') {
	$sort_method = 'ORDER by  ci.arrival_date asc, ci.item_code, ci.lot_ship  ';
}
elseif  ($sort1 == 'ship_dt'){
	$sort_method = 'ORDER by  ci.ship_date asc, ci.item_code, ci.lot_ship  ';
}
else {
   ...
   $query = "SELECT id.item_code,   ci.lot_ship, id.weight as bag_lbs,
          ...
          and ci.item_active != 1          
           AND ci.contract_date Between \"$from_date\" and \"$to_date\"
           and ci.warehouse = 'C'
	      $sort_method ";
