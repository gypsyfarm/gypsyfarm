...
echo '<form method=POST action="admin_coffee_commit_test.php">';
...
#	echo "warehouse value is: $warehouse";
...
$warehouse_where = "";
if ($warehouse <> '%') {
	$warehouse_where = " and c2.fob_code = '$warehouse' ";
}
...
$order_warehouse = "";
if ($warehouse <> '%') {
	$order_warehouse = " and o.warehouse = '$warehouse' ";
}
...
AND i.item_code = '$item' $order_warehouse ";
...
echo '&nbsp;&nbsp;&nbsp;<font size=3 color=blue>Now viewing commitments for,&nbsp;'.$item.'&nbsp;for the year, '.$year_range.'<br>';
...
echo '<th align=center><font color=blue>Company</font></th>';
...
$query = "select item_code, Notes, notes_date, notes_operator, ct.Company  
            from $tbl_coop_commited cc
            left join $tbl_coop_contact ct on  ct.contact_id =  cc.customer_key
...
for ($i=0; $i <$num_results;  $i++)
  {
       ...
       $print_desc = $print_desc."<br>Notes Updated by ".$row['notes_operator']." On:".$row['notes_date'];
        ...
        echo "<tr><td><font color=blue>$print_code</font></td>";
                echo "<td><font color=blue>$company</font></td>";
