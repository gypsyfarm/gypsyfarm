...
<br><center><h1>Review Orders</h1></center><br><br><br><br>
...
require("../tables.php");
  ...
  if (isset($_SESSION['valid_user']))
  {
	...
	echo '<bold><font size=4>Reviewing Orders for ', $user,'</font></bold>';
  }
  else
  {
 	header("Location: http://www.coopecoffeesbeans.com/badlogin.php");
  }
...
//*****************************************************************************
   ...
   $query = "SELECT cc.company, cc.Warehouse, cc.ShipCity, cc.name, oh.ship_date,
    oh.customer_key, oh.header_id, oh.order_date, oh.fbo_city, oh.order_nbr
   ...
   if ($num_results==0)   {
   echo '<br><font size=3 color=red>Your organization has no orders to review</font>';
   }
   else {
   echo '<br><font size=3 color=red>To date, your organization has placed '.$num_results.' orders with Cooperative Coffees</font>';
...
//***********Draw the TABLE*************************************************************
   ...
   for ($i=0; $i <$num_results; $i++)
   {
	...
	echo "<td><font color=black>".$row['fbo_city']."&nbsp;</font></td>";
...
$query = "select oh.header_id, oh.order_date, li.item_id, li.lot_id, li.lot_ship,
          oi.cost, oi.quantity AS item_quantity,  li.quantity AS lot_quantity,  oi.bag_lbs,
   ...
   if ($num_results==0)   {
   echo '<br><font size=3 color=red>Please Select an order to see the associated items</font>';
   }
   else {
    echo '<br><font size=3 color=red>There are '.$num_results.' items associated with this order</font>';
   	echo " <TABLE cellSpacing=0 cellPadding=0 width='95%' border=1>";
    echo '<tr>';
...

   {
	...
	echo "<td><font color=black>".$row['item_quantity']."&nbsp;</font></td>";
	echo "<td><font color=black>".$row['bag_lbs'] * $row['item_quantity']."&nbsp;</font></td>";
	echo "<td><font color=black>".$row['item_description']."&nbsp;</font></td>";
	echo "<td><font color=black>".$row['mark']."&nbsp;</font></td>";
 	echo "</tr>";
	$total_bags = $total_bags + $row['item_quantity'];
   }

 	echo "</table>";
	echo "<font size=3 color=red><bold>Total amount of bags ordered = $total_bags</bold></font>";
 	echo "<p>";
 }
}
?>
