...
if (isset($HTTP_SESSION_VARS['contact_id']))  {
    ...
    echo 'Orders Waiting for approval';
    echo '<center>'; 
    echo '<form name="frmMain" action=bankorder1.php method=post>';
    ...
    $query = "SELECT cc.company, cc.name, oh.header_id, oh.order_nbr, oh.order_date
              ...
              AND (oh.status = 'B' or oh.status = 'H')
  ...
  #  echo '<br>'.$query.'<br>';
   ...
   for ($i=0; $i <$num_results; $i++)
   {
      ...
      echo "&nbsp;&nbsp;&nbsp;<font size=2 color=blue><a href='bankorder2.php?order_id=".$row['header_id']."'>".$row['header_id']."(".$row['order_date'].")</font></a>\n";
      $row = mysql_fetch_array($result);
